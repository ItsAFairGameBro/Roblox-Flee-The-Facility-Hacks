--SCRIPT GITHUB LINK:
local githubID = "https://github.com/ItsAFairGameBro/Roblox-Flee-The-Facility-Hacks/blob/main/Main.lua"
...
function ReloadFunction()
	...
	local success, response = pcall(requestFunction,requestCaller,githubID,false)

	if not success then
		return warn(PrintName.." Error Loading Script: "..response)
	end
	...
	local scriptName = decodedJSON.payload.blob.displayName

	local success3, codeString = pcall(table.concat, decodedJSON.payload.blob.rawLines, "\n")
	...
	print(("%s %s Callback: %s (%.2fs)"):format(PrintName,scriptName,Callback,os.clock()-StartTime))
end
